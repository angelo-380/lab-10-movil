...
import com.example.lab_10.data.SerieModel
...
fun ContenidoSeriesListado(navController: NavHostController, servicio: SerieApiService) {
    ...
    // 👈 ESTE ERA EL DETALLE: Protegido con try-catch para evitar cierres inesperados
    LaunchedEffect(Unit) {
        try {
            ...
            Log.e("API_ERROR", "Error al obtener series: ${e.message}")
        }
    }
    ...
    if (id != 0) {
        LaunchedEffect(Unit) {
            try {
                ...
                Log.e("API_ERROR", "Error al cargar serie individual: ${e.message}")
            }
        }
    }
    ...
    if (grabar) {
        ...
        LaunchedEffect(Unit) {
            try {
                ...
                Log.e("API_ERROR", "Error al guardar serie: ${e.message}")
            } finally {
                grabar = false
    ...
    if (borrar) {
        LaunchedEffect(Unit) {
            try {
                ...
                Log.e("API_ERROR", "Error al eliminar serie: ${e.message}")
            } finally {
                borrar = false
                navController.navigate("series")
            }
